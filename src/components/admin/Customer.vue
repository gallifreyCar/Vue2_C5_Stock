...
<!--        搜索添加-->
          <el-col :span="10" >  <!--列宽-->
            <el-input placeholder="请输入内容"  v-model="queryInfo.query" clearable @click="getCustomerList">
              <el-button slot="append" icon="el-icon-search" @click="getCustomerList">
        ...
        <!--表单列表   border边框   stripe隔行变色-->
        <el-table :data="CustomerList" border stripe>

          <el-table-column type="index"></el-table-column> <!--索引列-->
          <el-table-column label="客户编号" prop="cno"></el-table-column>
          <el-table-column label="客户姓名" prop="cname"></el-table-column>
          <el-table-column label="客户地址" prop="cname"></el-table-column>
          <el-table-column label="联系方式" prop="ctel"></el-table-column>
          <el-table-column label="客户密码" prop="cpw"></el-table-column>
          ...
          <el-table-column label="操作" >
            <template slot-scope="scope">
              ...
              <el-button type="danger" icon="el-icon-delete" size="mini" @click= "deleteCustomer(scope.row.cno)"></el-button>
              </el-tooltip>
...
<!--        分页组件 size-change每页最大变化数  current-change当前变化数 layout功能组件-->
    ...
    <!--修改表单表对话框-->
    <el-dialog title="修改表单" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
  ...
  methods:{
    ...
    addUser(){
      this.$refs.addFormRef.validate(async valid=>{
            ...
            // 验证后台，不成功返回
             if(!valid) return;
             const{data:res}=await this.$http.post("getCustomer",this.addForm);
             if(res!="success"){
    ...
    async deleteCustomer(cno){
     const confirmResult =await this.$confirm("此操作将永久删除用户，是否继续？",'警告',{
      ...
      const{data:res}=await this.$http.post("deleteCustomer?cno="+cno);
      console.log(res);
      if(res!="success"){
        return this.$message.error("操作失败");
      }
