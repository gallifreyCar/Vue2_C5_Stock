...
<!--        搜索添加-->
          <el-col :span="10" >  <!--列宽-->
            <el-input placeholder="请输入内容"  v-model="queryInfo.query" clearable @click="getStockList">
              <el-button slot="append" icon="el-icon-search" @click="getStockHouseList">
        ...
        <el-table :data="StockHouseList" border stripe>

          <el-table-column type="index"></el-table-column> <!--索引列-->
          <el-table-column label="仓库编号" prop="sno"></el-table-column>
          <el-table-column label="仓库名字" prop="sname"></el-table-column>
          <el-table-column label="仓库地址" prop="saddr"></el-table-column>
          ...
          <el-table-column label="操作" >
            <template slot-scope="scope">
              ...
              <el-button type="danger" icon="el-icon-delete" size="mini" @click= "deleteStockHouse(scope.row.sno)"></el-button>
              </el-tooltip>
  ...
  methods:{
    ...
    async deleteStockHouse(sno){
     const confirmResult =await this.$confirm("此操作将永久删除表单，是否继续？",'警告',{
      ...
      const{data:res}=await this.$http.post("deleteStockHouse?sno="+sno);
      console.log(res);
      if(res!="success"){
        return this.$message.error("操作失败");
      }
